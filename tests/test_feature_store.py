...
class TestFeatureStore:
    ...
    def test_concurrent_ingest(self, store):
        ...
        threads = [threading.Thread(target=_ingest, args=(f"concurrent_{i }",)) for i in range(10)]
        for t in threads:
            t.start()
        for t in threads:
            t.join()
...
class TestRedisIntegration:
    ...
    def test_pipeline_batch_write(self, redis_store):
        snaps = [
            FeatureSnapshot(entity_id=f"batch_{i }", computed_at=time.time()) for i in range(5)
        ]
