...
async def kafka_ingest_loop(store: FeatureStore, bootstrap: str) -> None:
    ...
    async for txn in consumer:
        ...
        if time.time() - t_report >= 5.0:
            ...
            logger.info(
                "Ingested %d events | Store writes: %d | ~%.0f RPS",
                ingested,
                store.health()["writes"],
                rps,
            )
...
async def _main(args: argparse.Namespace) -> None:
    ...
    if args.simulate:
        ingest_task = asyncio.create_task(simulator_ingest_loop(store, target_rps=args.rps))
    else:
        ingest_task = asyncio.create_task(kafka_ingest_loop(store, bootstrap=args.kafka))
