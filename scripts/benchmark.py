...
def bench_feature_read (store :FeatureStore ,n :int =5_000 )->None :
    ...
    label ="Feature retrieval (L2 Redis)"if store ._l2 .available else "Feature retrieval (live compute)"
...
def bench_inference_inprocess (store :FeatureStore ,n :int =5_000 )->None :
    ...
    for _ in range (50 ):
        core .predict_single (
        entity_id =random .choice (entity_ids ),transaction_id ="warmup",
        amount =100.0 ,lat =40.7 ,lon =-74.0 ,merchant_category ="grocery",
        )
    ...
    def _predict (_ ):
        t0 =time .perf_counter ()
        core .predict_single (
        entity_id =random .choice (entity_ids ),transaction_id ="bench",
        amount =random .uniform (5 ,1000 ),lat =40.7 ,lon =-74.0 ,
        )
        with lock :
